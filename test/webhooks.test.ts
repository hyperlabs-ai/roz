...
import { verifyGithub, verifyLinear } from '../src/utils/webhooks.js';
...
describe('verifyLinear', () => {
  it('acepta una firma válida (hex sin prefijo)', () => {
    const sig = createHmac('sha256', secret).update(body).digest('hex');
    expect(verifyLinear(body, sig, secret)).toBe(true);
  });
  it('rechaza una firma inválida', () => {
    expect(verifyLinear(body, 'nope', secret)).toBe(false);
  });
});
